...
form field choices, and formset behavior.
It ensures forms render and validate correctly.
"""
...
class EntryFormTests(TestCase):
    """Tests for EntryForm: initialisation, widgets, and
    field configuration.
    """

    def test_initial_date_for_new_entry(self):
        """Verify that the date field defaults to the current time
        for new entries.
        """
        form = EntryForm()
    ...
    def test_widget_and_choices(self):
        """Verify form field widgets and choice options are configured
        correctly.

        Checks that:
        - Date field uses DateTimeInput widget
        - Mood field has correct mood choices
        - Mood rating field uses Select widget with correct 1-5 choices
        """
        form = EntryForm()
        ...
        # `mood_rating` is an IntegerField with a Select widget;
        # check widget choices
        self.assertIsInstance(form.fields["mood_rating"].widget, Select)
        self.assertEqual(
            form.fields["mood_rating"].widget.choices,
            MOOD_RATING_CHOICES,
        )
...
class GratitudeFormsetTests(TestCase):
    ...
    def test_gratitude_formset_extra(self):
        """Verify GratitudeFormSet provides 3 extra blank forms for
        entry creation.

        The formset is used during entry creation to allow users to add
        3 gratitude items.
        """
    ...
    def test_gratitude_edit_formset_no_extra(self):
        """Verify GratitudeEditFormSet provides no extra blank forms
        during editing.

        The edit formset shows only existing gratitude items without
        extra blanks, preventing unintended item creation during an edit.
        """
